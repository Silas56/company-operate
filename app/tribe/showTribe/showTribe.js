...
$scope.savaModifyData=function(){
        ...
        var allInputs = angular.element(document.getElementsByTagName('input'));
			...
			if( allSavaTrue.length===0){
				showMessage(ngDialog,"支持多个权限，最少选中一项");
                $scope.actions.setChildCurrent(2);
				return;
			}else{
                $scope.actions.setChildCurrent(3);
			}
...
//配置分页基本参数默认设置
    ...
    $scope.paginationConfForRufuse = {
        itemsPerPage: 10,
        currentPage: 1
    };
    $scope.paginationConfForLog = {
        itemsPerPage: 10,
        currentPage: 1
    };
...
// 申请加入接口============================================================ 
	...
	$scope.switchStringForshopType = function(num){
		if(num == 1){
			return '酷鸟商家' ;
		}else if(num == 2){
			return '酷鸟分销商' ;
    ...
    $scope.switchStringForshopStatus = function(num){
        if(num == 1){
            return '审核中' ;
        }else if(num == 2){
            return '正常营业' ;
...
$scope.start_refuse=function(){
		...
		$scope.param_refuse.pageNum = $scope.paginationConfForRufuse.currentPage;
		$scope.param_refuse.pageSize = $scope.paginationConfForRufuse.itemsPerPage;
...
//  console.log("部落拒绝列表读取成功",res);
				...
				// console.log(res.tribeInfoPage.total,"拒绝列表total");
				$scope.paginationConfForRufuse={
	...
	$scope.$watch('paginationConfForRufuse.currentPage + paginationConfForRufuse.itemsPerPage', $scope.start_refuse);
...
// 启动函数
	$scope.start_log=function(){
		...
		$scope.param_log.pageNum = $scope.paginationConfForLog.currentPage;
		$scope.param_log.pageSize = $scope.paginationConfForLog.itemsPerPage;
...
// console.log("部落日志列表读取成功",res)
	...
	$scope.$watch('paginationConfForLog.currentPage + paginationConfForLog.itemsPerPage', $scope.start_log);
